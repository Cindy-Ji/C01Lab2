    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (!title && !content) {
        return res.status(400).json({
          error: "At least one of title or content is required for update.",
        });
      }
      ...
      if (result.modifiedCount > 0) {
        res.status(200).json({
          response: `Document with ID ${noteId} successfully updated.`,
        });
      } else {
        res.status(400).json({
          error: `No changes detected for note with ID ${noteId}.`,
        });
      }
    });
  } catch (error) {
